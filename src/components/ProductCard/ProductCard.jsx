  ...
  return (
    <article className={styles.card} tabIndex={0} aria-labelledby={`p-${product.id}-title`}>
      <div className={styles.cardTop}>
        <button className={styles.thumb} onClick={() => onPreview(product)} aria-label={`Preview ${product.name}`}>
        {/* TODO: replace with product image */}
        <div className={styles.thumbPlaceholder}>Photo</div>
        </button>

        <button className={`${styles.heart} ${isFavorite ? styles.fav : ''}`} aria-pressed={isFavorite} aria-label={isFavorite ? 'Remove from favorites' : 'Add to favorites'} onClick={() => onToggleFavorite && onToggleFavorite(product.id)}>
          ♥
        </button>
      </div>

      <div className={styles.rowTop}>
        <h3 id={`p-${product.id}-title`} className={styles.title} onClick={() => onPreview(product)}>{product.name}</h3>
